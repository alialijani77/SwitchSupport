...
namespace SwitchSupport.Application.Services.Implementions.Question
{
    public class QuestionService : IQuestionService
    {
        ...
        public async Task<List<TagJsonViewModel>> GetTagListForChartJs()
        {
            var tags = await _questionRepository.GetAllFilterTags();

            var res = tags.OrderByDescending(t => t.UseCount).Take(10).Select(t => new TagJsonViewModel
            {
                Title = t.Title,
                UseCount = t.UseCount
            }).ToList();
        ...
        public async Task<EditTagAdminViewModel> getTagForEditTagAdmin(long tagId)
        {
            ...
            var editTag = new EditTagAdminViewModel();
            editTag.Id = tagId;
            editTag.Title = tag.Title;
            editTag.Description = tag.Description;
            return editTag;
        }

        public async Task EditTagAdmin(EditTagAdminViewModel editTag)
        {
